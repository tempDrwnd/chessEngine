...
public abstract class Piece {
    ...
    Returns: Whether one can move a that piece to that square legally. true if yes, false if no
     */
    public static boolean isFreeSquare(int square, int origin, boolean isWhite, String board) {
        ...
        return !isInCheck(isWhite, Piece.move(origin, square, board));  //Returns true if one wouldn't be in check after making that move
    }
    ...
    Empty square is classified as black, so that always needs to be checked manually
    ...
    /*
    Makes a specified move in the specified board and appends that move to the following position
    ...
    public static String move(int origin, int target, String board) {
        StringBuilder s = new StringBuilder(board.substring(0, 64));    //String builder initialized with only the position, not the last move
        ...
        if (s.charAt(target) == 'p' || s.charAt(target) == 'P') {   //Is a pawn move
            ...
            boolean hasLineMovement = Math.abs((origin % 8) - (target % 8)) == 1;   //Makes sure the pawn moved horizontally only a single square
            //Makes sure the pawn moved forward only a single square  && last move was a pawn
            if ((target >> 3) - (origin >> 3) == 1 && hasLineMovement && board.charAt(lastMove % 64) == 'p'
                    //Last move was two square forward                                                && you move to the same file as the pawn from the last move
    ...
    public static void promote(char piece, int pos) {
        ...
        if(Main.botPlays) {
            Main.makeBotMove(Main.testBot2);    //Makes a bot move if you play against a bot
        }
